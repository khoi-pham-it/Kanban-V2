...
Khi người dùng thêm một Card mới với Zustand:
\begin{enumerate}
    \item Nhập tiêu đề vào Component UI \texttt{AddCardForm}.
    \item Lấy action từ Store bằng hooks: \texttt{const \{ addCard \} = useBoardStore()}.
    \item Gọi \texttt{addCard(boardId, listId, title)}.
    \item \texttt{Zustand} kết hợp \texttt{Immer} thực hiện thay đổi mảng State một cách bất biến.
    \item React tự động đối chiếu Virtual DOM và re-render chính xác \texttt{List} chứa Card vừa thêm.
\end{enumerate}
